...
| M5 App 骨架 + 服务器配置 + 登录 | ✅ 完成 | `e4cec89` | Kotlin/Compose 导航/Retrofit+OkHttp+Moshi/无DI(ServiceLocator)/服务器地址配置(默认110.42.251.26:8000+恢复默认)/注册→验证→登录/App图标。模拟器验证通过 |
...
| 服务器地址 | 默认 110.42.251.26:8000，用户可改+恢复默认 | 用户指定（M5） |
...
## 9. 下一步：M9（活跃度计算与上报）计划要点

参考 `安心圈编程计划.md` 的 M9（M5-M8 已完成）：
- 活跃度计算：归一化（M8 已产出 0..1 值）+ 权重加权求和（PRD §4.3 默认权重，可调）
- 按好友数据源组合计算可见活跃度（PRD §4.2，后端 M3 已实现后端侧计算，M9 前端侧上报原始分项由后端按矩阵算）
- WorkManager 每 30 分钟上报（POST /activity/report）
- 响应处理：刷新本地好友状态、写入 30 天历史
- 本地历史清理：超 30 天自动删除
- 权重设置页（用户调整各数据源贡献权重）

> M8 已产出 7 源 0..1 归一化值存 SnapshotStore；M9 读取后上报 /activity/report，后端按好友矩阵计算可见值并下发。
> 注意：后端 M3 决策是「后端按矩阵计算」，所以前端上报原始 components 即可，无需前端按好友拆分。
