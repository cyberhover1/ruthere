...
参考 `安心圈编程计划.md` 的 M8（M5-M7 已完成）：
- 7 类数据源采集器：步数/屏幕解锁/充电/耳机/拿起翻转/环境光/显著运动
- 统一采集器接口，便于按好友组合开关
- 运行时权限申请（ACTIVITY_RECOGNITION 等）
- 各传感器用 Android SensorManager / ActivityRecognitionClient / BatteryManager / AudioManager 等
- 采集到的原始数据供 M9 计算活跃度

> M8 是前端核心功能的传感器层，M9 才用采集数据计算并上报活跃度。
