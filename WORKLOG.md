...
## 9. 下一步：M7（前端好友系统）计划要点

参考 `安心圈编程计划.md` 的 M7（M5/M6 已完成）：
- 好友列表页（头像/昵称/活跃度/模糊时间）—— M5 已有占位，M7 接真实数据
- 添加好友页：生成动态二维码、扫码加好友（ZXing）、邮箱搜索+发送申请、申请列表（同意/拒绝）
- 好友详情页：修改昵称、删除好友（删除提示展示）、设置数据源组合
- 权重设置页：用户调整各数据源贡献权重
- 排序：按活跃度 / 最后更新时间 / 昵称

> 前端 API 层（Retrofit+Moshi+JWT拦截器）已在 M5 搭好，M7 扩展 ApiService 加 friends 端点即可。
> M8/M9 才触及核心传感器采集与活跃度计算。
